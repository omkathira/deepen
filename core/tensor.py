...
from deepen.ops.utils import _compute_initializer_fans, _make_cache
...
class Tensor:
    ...
    def __add__(self, other):
        return Tensor._from_op(add, self, other)

    def __radd__(self, other):
        return Tensor._from_op(add, other, self)

    def __sub__(self, other):
        return Tensor._from_op(sub, self, other)

    def __rsub__(self, other):
        return Tensor._from_op(sub, other, self)

    def __mul__(self, other):
        return Tensor._from_op(mul, self, other)

    def __rmul__(self, other):
        return Tensor._from_op(mul, other, self)

    def __truediv__(self, other):
        return Tensor._from_op(div, self, other)

    def __rtruediv__(self, other):
        return Tensor._from_op(div, other, self)

    def __neg__(self):
        return Tensor._from_op(neg, self)

    def __abs__(self):
        return Tensor._from_op(abs_, self)

    def __pow__(self, power):
        return Tensor._from_op(pow_, self, power)

    def exp(self):
        return Tensor._from_op(exp, self)

    def log(self, base=None):
        return Tensor._from_op(log, self, base=base)

    def clip(self, min_val, max_val):
        return Tensor._from_op(clip, self, min_val=min_val, max_val=max_val)

    # Logical operations
    def __eq__(self, other):
        return Tensor._from_op(eq, self, other)

    def __ne__(self, other):
        return Tensor._from_op(ne, self, other)

    def __lt__(self, other):
        return Tensor._from_op(lt, self, other)

    def __le__(self, other):
        return Tensor._from_op(le, self, other)

    def __gt__(self, other):
        return Tensor._from_op(gt, self, other)

    def __ge__(self, other):
        return Tensor._from_op(ge, self, other)

    def __invert__(self):
        return Tensor._from_op(not_, self)

    def __and__(self, other):
        return Tensor._from_op(and_, self, other)

    def __or__(self, other):
        return Tensor._from_op(or_, self, other)

    # Index operations
    def gather(self, indices=None):
        return Tensor._from_op(gather, self, indices=indices)

    # Shape operations
    def squeeze(self, axes=None):
        return Tensor._from_op(squeeze, self, axes=axes)

    def unsqueeze(self, axes=None):
        return Tensor._from_op(unsqueeze, self, axes=axes)

    def transpose(self, axes=None):
        return Tensor._from_op(transpose, self, axes=axes)

    def concatenate(self, other, axes=None):
        return Tensor._from_op(concatenate, self, other, axes=axes)

    def reshape(self, *shape):
        return Tensor._from_op(reshape, self, shape=shape)

    # Reduction operations
    def sum(self, axes=None):
        return Tensor._from_op(sum_, self, axes=axes)

    def mean(self, axes=None):
        return Tensor._from_op(mean, self, axes=axes)

    def min(self, axes=None):
        return Tensor._from_op(min_, self, axes=axes)

    def max(self, axes=None):
        return Tensor._from_op(max_, self, axes=axes)

    def softmax(self, axes=None):
        return Tensor._from_op(softmax, self, axes=axes)

    # Linear algebra operations
    def matmul(self, other):
        return Tensor._from_op(matmul, self, other)

    def outer(self, other):
        return Tensor._from_op(outer, self, other)

    # Activation functions
    def sigmoid(self):
        return Tensor._from_op(sigmoid, self)

    def tanh(self):
        return Tensor._from_op(tanh, self)

    def relu(self):
        return Tensor._from_op(relu, self)

    def leaky_relu(self, neg_slope=0.1):
        return Tensor._from_op(leaky_relu, self, neg_slope=neg_slope)

    def swish(self):
        return Tensor._from_op(swish, self)
